...
function kcSendEmail($data)
{
    ...
    if (!empty($check_exist_post)) {
        ...
        if (isset($data['attachment']) && $data['attachment']) {
            $email_status = wp_mail($data['user_email'], $email_title, html_entity_decode($email_content), $headers, isset($data['attachment_file']) ? $data['attachment_file'] : '');
        } else {
            $email_status = wp_mail($data['user_email'], $email_title, html_entity_decode($email_content), $headers);
        }
            ...
            if (is_array($data['service'])) {
                if(!empty($data['widgetType']) && $data['widgetType'] === 'phpWidget'){                  
                    $service_id = array_map(function ($v) {
                        if(is_string($v)){
                            $serviceId = json_decode(stripslashes($v))->service_id;
    ...
    <div>
        ...
        <style>
                ...
                .container-kivicare {
                    ...
                    /* Remove flex/height that push footer to next page */
                    /* display: flex; */
                ...
                .main-content {
                    /* Let content flow naturally so it doesn't trigger page break */
                    display: block;
                }
                ...
                /* Reduce excessive padding and tighten layout */
                .container-kivicare table { width: 100%; border-collapse: collapse; }
                .container-kivicare td, .container-kivicare th { padding: 4px 0 !important; vertical-align: top; }
                header table td, header table th { padding: 4px 0 !important; }

                /* Keep important blocks on the same page */
                .no-break { page-break-inside: avoid; break-inside: avoid; }

                /* Signature block styling inside main content */
                .signature-block { margin-top: 12px; text-align: right; }
                .signature-block .label { font-weight: 700; color: #171C26; margin-bottom: 6px; }
                .signature-block img.signature-img { max-height: 60px; width: auto; display: inline-block; }
                ...
                * {
                ...
                /* Optional: compress page margins slightly */
                @page { margin: 12mm; }
            }
        </style>
            ...
            <div style="padding: 10px;">
                ...
                <div class="main-content">
                    ...
                    <?php if ($type === 'encounter') { ?>
                        <div class="container-fluid signature-block no-break">
                            <hr class="border-top-5" style="margin: 8px 0;">
                            <div class="row" style="margin: 0;">
                                <div class="col-8"></div>
                                <div class="col-4 text-center" style="padding: 0;">
                                    <p class="label"><?php echo esc_html__('Doctor Signature', 'kc-lang') ?></p>
                                    <?php
                                    $signature = get_user_meta($data->doctor_id, 'doctor_signature', true);
                                    if (!empty($signature)) {
                                        ?>
                                        <img class="signature-img" src="<?php echo esc_html($signature); ?>" alt="signature">
                                        <?php
                                    }
                                    ?>
                                    <p class="border-bottom border-dark" style="margin: 6px 0 0 0;"></p>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
    ...
    if (!empty($data->include_encounter_custom_field) && $data->include_encounter_custom_field == 'true') {
        ...
        if (!empty($custom_field) && is_array($custom_field)) {
            foreach ($custom_field as $cf) {
                if (!empty($cf['field_data'])) { $has_custom_field_data = true; break; }
            }
        }
